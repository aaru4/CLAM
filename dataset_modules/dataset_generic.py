...
class Generic_MIL_Survival_Dataset(Generic_WSI_Classification_Dataset):
	"""
	Survival dataset — returns (features, tensor([bin, event])) per slide.
	...
	def __getitem__(self, idx):
		...
		event    = int(self.slide_data['event'].iloc[idx])
	...
	def _get_survival_split(self, all_splits, split_key):
		...
		if len(split) > 0:
			mask = self.slide_data['slide_id'].isin(split.tolist())
			df_slice = self.slide_data[mask].reset_index(drop=True)
			return Generic_Survival_Split(df_slice, data_dir=self.data_dir, num_classes=self.num_classes)
		return None
...
class Generic_Survival_Split(Generic_MIL_Survival_Dataset):
	"""Split class for survival — mirrors Generic_Split but returns (features, tensor([bin, event]))."""
